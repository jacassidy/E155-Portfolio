...
- Extend the course reference designs with **new hardware modules** including SPI-based telemetry, line-clear logic, and debug overlays.
...
- An **FPGA** responsible for:
  ...
  - Input decoding from a PS/2 keyboard.
  - Telemetry overlay and debug visualization on top of the game board.

Communication between the MCU and FPGA uses a custom **SPI-based protocol**, with the MCU as master. The FPGA exposes memory-mapped registers for commands, random-number injection, and telemetry readback.
...
An on-chip PLL generates the required pixel clock for 640×480@60 Hz VGA from the external oscillator. All VGA timing and game logic are synchronized to this clock domain, ensuring a stable display with no visible jitter or tearing. The design avoids derived or gated clocks; instead, it uses enables in a single clock domain, which improves timing closure and reliability.
...
- Provides a `visible` flag so downstream modules only draw within the active video region.
...
- **Collision Detection**
  Uses the current board representation and the active piece mask to detect collisions with:
  - Walls and floor.
...
- Left / Right movement
- Rotation
- Soft drop / hard drop (if enabled)
- Reset / pause (optional)
...
A `telemetry_module` renders text or numeric values into a reserved region of the screen. Internally:

- Signals such as score, lines cleared, current piece ID, and frame count are packed into a telemetry bus.
- The module converts these values into characters and overlays them after the game board is drawn, without affecting the board’s underlying representation.
...
Schematics are neatly drawn with a CAD program (e.g., KiCad) and include:
...
| Item / RefDes | Description                                | Manufacturer / Part # | Qty | Unit Price (USD) | Ext. Price (USD) |
|---------------|--------------------------------------------|-----------------------|-----|------------------|------------------|
<!--
| U1            | FPGA development board                     |                       | 1   |                  |                  |
| U2            | Microcontroller development board          |                       | 1   |                  |                  |
| J1            | VGA connector (HD-15)                      |                       | 1   |                  |                  |
| J2            | PS/2 connector                             |                       | 1   |                  |                  |
| R*            | Resistor ladder for VGA DAC                |                       |     |                  |                  |
| C*            | Decoupling capacitors                      |                       |     |                  |                  |
| ...           | ...                                        |                       |     |                  |                  |
-->

You can add subtotals if needed:

- **Electronics subtotal:**
- **Mechanical / misc subtotal:**
- **Total project cost:**
